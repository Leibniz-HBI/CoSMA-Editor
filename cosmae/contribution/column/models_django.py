"ORM models for columns of contribution candidates."

from django.db import models


class ColumnContribution(models.Model):
    "ORM Model for newly added contribution columns"
    ...
    def get_by_candidate_query_set(cls, candidate):
        "Get all columns for a contribution candidate."
        return ColumnContribution.objects.filter(  # pylint: disable=no-member
            contribution_candidate=candidate,
        )

    @classmethod
    def get_by_id_persistent(cls, id_persistent: str, contribution_candidate):
        "Get a contribution column by id_persistent and user"
        return ColumnContribution.objects.filter(  # pylint: disable=no-member
            id_persistent=id_persistent, contribution_candidate=contribution_candidate
        ).get()


class ValueContribution(models.Model):
    "ORM model for values of contributions candidates."

    value = models.TextField()
    id_entity_persistent = models.TextField(null=True, blank=True)
    column = models.ForeignKey(ColumnContribution, on_delete=models.CASCADE)
