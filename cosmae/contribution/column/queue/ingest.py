    ...
    try:
        with transaction.atomic():
            try:
                ...
                return
            active_columns = (
                ColumnContribution.objects.filter(  # pylint: disable=no-member
            ...
            for column_assignment in active_columns:
                if column_assignment.id_existing_persistent == "display_txt":
                    ...
                    merge_request_base_name = (
                        column_destination.name + " Merge Request " + contribution.name
                    )
                    merge_request_name = merge_request_base_name
                    column_origin = None
                    id_column_origin_persistent = str(uuid4())
                    # loop is for fallback in case the column already exists
                    for idx in range(1, 10):
                        try:
                            (
                                ...
                                id_persistent=id_column_origin_persistent,
                                name=merge_request_name,
                                written_by_session=contribution.edit_session,
                                id_parent_persistent=(
                                    column_destination.id_parent_persistent
                                ),
                                type=column_destination.type,
                                time_edit=time_add,
                    ...
                    column_assignments.append(
                        (column_assignment.index_in_file, column_origin)
                    )
                    column_pairs.append((column_origin, column_destination))
            ...
            for row_tpl in data_frame.itertuples(index=False):
                ...
                for idx_in_file, column in column_assignments:
                    ...
                    value, _ = ValueHistory.change_or_create_versioned(
                        id_persistent=id_value_persistent,
                        id_entity_persistent=id_entity_persistent,
                    ...
                    value.save()
            for origin, destination in column_pairs:
                ColumnMergeRequest(
                    id_persistent=uuid4(),
        ...
        with transaction.atomic():
            ...
            contribution_candidate.set_state(
                ...
                "Error during ingestion of assigned columns.",
                exc,
            )
