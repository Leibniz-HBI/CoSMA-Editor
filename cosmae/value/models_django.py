...
class Value(ValueAbstract):
    ...
    @classmethod
    def for_entities(
        cls, id_column_persistent: str, id_entity_persistent_list: List[str]
    ):
    ...
    @classmethod
    def by_column_chunked_queryset(
        cls, id_column_persistent, offset, limit, manager=None
    ):
    ...
    def most_recent_by_entity_and_definition_id_query_set(
        cls, id_entity_persistent: str, id_column_persistent: str
    ):
    ...
    def for_entity_queryset(
        cls, id_entity_persistent: str, _user: CosmaeUser, include_disabled: bool = False
    ):
    ...
    @classmethod
    def annotate_entity(cls, manager: Optional[models.BaseManager[Value]]):
        "Annotate values with the most recent entity and value version"
        if manager is None:
            manager = Value.objects  # pylint: disable=no-member
    ...
    @classmethod
    def annotate_column(cls, manager: Optional[models.BaseManager[Value]]):
