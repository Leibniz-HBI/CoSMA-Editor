...
def get_display_txt_order_columns(id_contribution_persistent=None):
    "Retrieve the order of columns used for display text alternatives."
    id_column_persistent_list = ConfigValue.get(DISPLAY_TXT_ORDER_CONFIG_KEY, [])
    if id_contribution_persistent is None:
        order = [
            When(id_persistent=val, then=idx)
            for idx, val in enumerate(id_column_persistent_list)
        ]
        column_query = Column.query_set().filter(
            id_persistent__in=id_column_persistent_list
        )
        ...
        relevant_for_display_txt_order = merge_requests.filter(
            id_destination_persistent__in=id_column_persistent_list
        )
        column_query = (
            Column.query_set()
            .annotate(
