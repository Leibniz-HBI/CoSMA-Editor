...
const helpMap: HelpMap = {
    '': {
        '': `# View Data
## About
This is the landing page of **CoSMA-Editor**. The following will explain the possible action that can be performed on this page.
...
### Columns
Keep track of column ownership requests you are involved in.
### Logout
...
By clicking the plus icon at the top right of the table a popup will open from where you can select additional columns to display.
### Add Entity
...
Here you can assign the columns of a \`csv\` upload to new or existing columns in the trustee.
## List of columns
...
## Assignment View
In the center you will find the list of existing columns.
To assign the selected column from the csv to an existing column click the radio button on the right of a column entry.
If you want to create a new column click on **Create new column**
## Preview
In the future you will be able to compare data from the selected column and the assigned column.
...
* The third row show how many columns have an exact match for the contributed and existing entities. It is also blank for the contributed entity.
* The fourth row show the _display text_ of the entities.
* All remaining rows show values to aid in the comparison of contributed and matched entities. Exact matches of values are highlighted.
### Showing Additional Values
To further assist in comparing the contributed entity with existing ones in the trustee you can select additional values by clicking on **Show Additional Column Values**.
...
To the right are entity merge requests relevant to you.`,
            columns: {
                [variableKey]: `# Column Merge Request
This page contains all value conflicts between origin and destination columns.
...
## Deciding conflicts
For each conflict you have to decide wether to keep the value in the destination column or overwriting it with the value from the origin column.
## Applying resolutions
When all conflicts are resolved, click on **Apply Resolutions to Destination** apply the resolutions to the destination column.
`
            },
            entities: {
...
For conflicts you can not resolve, a column merge request is created.
...
By clicking on **Reverse Direction** the origin entity of the merge request becomes the destination and vice versa.`
        ...
        columns: {
            '': `# Column Ownership Request
Here you see the column ownership transfer requests you are involved in.
...
## Display Text Order
Control the order of fallback columns for entities without a display text.`,
            user: {
                '': `# User Permissions
...
## List of Fallback Columns for Display Text
When an entity has no display text, the trustee tries the values of the shown columns with decreasing priority from top to bottom.
If a value is found for the given column, the value is used as display text.
Otherwise the next column is tried.
If no column has a value for the given entity its persistent id is used as display text.
### Removing a Column from the Order
By clicking the cross on the right of an entry, the corresponding column is removed from the display text fallback order.
## Column Explorer
On the right is a column explorer.
By clicking on the plus symbol of an entry, the corresponding column is appended to the display text fallback order.`
            }
        }
    }
}
