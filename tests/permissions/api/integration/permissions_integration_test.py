...
def test_get_permissions_other_user(auth_server_commissioner, column):
...
def test_put_permissions_other_user(auth_server_commissioner, column):
...
def test_put_permissions_missing_user(auth_server, column):
    "Make sure correct status is returned for missing resource when setting permission."
    server, cookies = auth_server
    rsp = req.put_permission(
        server.url, c.id_column_persistent_test, cu.test_uuid1, cookies=cookies
...
def test_set_and_get_permissions_other_user(auth_server, column, user1, user_editor):
...
def test_can_remove_existing(auth_server, column, user1):
    "Make sure can set and get permissions"
    server, cookies = auth_server
    rsp = req.put_permission(
        server.url,
...
def test_can_set_partial(auth_server, column, user1):
    "Make sure can set and get permissions"
    server, cookies = auth_server
    rsp = req.put_permission(
        server.url,
...
def test_can_set_partial_initial(auth_server, column, user1):
