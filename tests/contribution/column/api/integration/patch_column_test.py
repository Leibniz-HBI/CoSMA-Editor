...
def test_404_column(auth_server):
    server, cookies = auth_server
    ...
    assert rsp.status_code == 404
    assert rsp.json()["msg"] == "Column does not exist."
...
def test_patch_id_unknown_special_column(auth_server):
    server, cookies = auth_server
    id_candidate_persistent, id_definition_persistent = candidate_id_with_extracted(
        server, cookies
    )
    ...
    assert rsp.status_code == 400
    assert rsp.json()["msg"] == "Existing column does not exist."
