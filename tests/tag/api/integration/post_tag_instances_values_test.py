...
def test_gets_most_recent(auth_server_commissioner, person, child_tag_def):
    ...
    rsp = post_column(live_server.url, child_tag_def, cookies=cookies)
    id_tag_def = rsp.json()["column_list"][0]["id_persistent"]
    assert rsp.status_code == 200
    rsp = post_value(
        live_server.url,
        {
            "id_entity_persistent": id_entity,
            "id_column_persistent": id_tag_def,
            "value": "1",
        },
        cookies=cookies,
    )
    ...
    rsp_instance = rsp.json()["value_list"][0]
    version = rsp_instance["version"]
    rsp = post_value_value(live_server.url, id_entity, id_tag_def, cookies=cookies)
    assert rsp.status_code == 200
    assert rsp.json() == {
        "value_responses": [
            {
