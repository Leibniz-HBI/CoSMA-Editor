...
def test_no_cookies(auth_server, tag_def_user):
    ...
    rsp = r.purge_tag(server.url, c.id_tag_def_persistent_test_user)
    assert rsp.status_code == 401
...
def test_bypasses_children(
    ...
    assert (
        len(
            TagDefinitionHistory.objects.filter(
                id_persistent=tag_def_parent.id_persistent
            )
        )
        == 0
    )
    for tag_def in [tag_def_child_0, tag_def_child_1]:
        tag_def_retrieved = TagDefinitionHistory.objects.filter(
