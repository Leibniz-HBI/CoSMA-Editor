...
def test_get_name_from_cache(tag_def):
    path = ["name", "path", "test"]
    q.column_name_path_cache.set(tag_def.id_persistent, path)
    result = q.get_column_name_path(tag_def)
    assert result == path
...
def test_name_as_default_path_and_enqueues(tag_def_user):
    ...
    with patch("cosmae.column.queue.enqueue", mock):
        result = q.get_column_name_path(tag_def_user)
    assert result == [tag_def_user.name]
    mock.assert_called_once_with(q.update_column_name_path, tag_def_user.id_persistent)


def test_enqueues_children(tag_def_parent, tag_def_child_0, tag_def_child_1):
    mock = MagicMock()
    with patch("cosmae.column.queue.enqueue", mock):
        q.update_column_name_path(tag_def_parent.id_persistent)
    assert q.column_name_path_cache.get(tag_def_parent.id_persistent) == [
        tag_def_parent.name
    ]
...
def test_uses_provided_parent_path(tag_def_parent, tag_def_child_0):
    ...
    from_cache = q.get_column_name_path(tag_def_child_0)
