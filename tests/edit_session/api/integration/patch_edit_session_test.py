...
def test_unknown_user(auth_server):
    ...
    with patch("cosmae.edit_session.api.check_user", mock):
        rsp = req.patch_edit_session(
            server.url, c.id_session_user, new_name, cookies=cookies
        )
...
def test_no_cookies(auth_server):
    ...
    rsp = req.patch_edit_session(server.url, c.id_session_user, new_name, cookies=None)
    assert rsp.status_code == 401
...
def test_change_name(auth_server):
