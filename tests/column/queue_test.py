...
def test_get_name_from_cache(column):
    path = ["name", "path", "test"]
    ColumnNamePathCache.set_cache_entry(column.id, path, column.time_edit)
    result = q.get_column_name_path(column, column.time_edit)
    assert result == path
...
def test_name_as_default_path_and_enqueues(column_user):
    ...
    with patch("cosmae.column.queue.enqueue", mock):
        result = q.get_column_name_path(column_user, column_user.time_edit)
...
def test_enqueues_children_no_future(column_parent, column_child_0, column_child_1):
    ...
    assert q.get_column_name_path(column_parent, column_parent.time_edit) == [
        column_parent.name
    ]
...
def test_enqueues_children_(column_parent_future, column_child_0, column_child_1):
    ...
    assert q.get_column_name_path(
        column_parent_future, column_parent_future.time_edit
    ...
    mock.assert_has_calls(
        [
            call(
                ...
                column_child_0.id,
                [column_parent_future.name],
                column_parent_future.time_edit,
            ),
            call(
                q.update_column_name_path,
                column_child_1.id,
...
def test_uses_provided_parent_path(column_parent, column_child_0):
    q.update_column_name_path(
        column_child_0.id, ["name", "path", "parent", "test"], column_parent.time_edit
    )
    from_cache = q.get_column_name_path(column_child_0, column_child_0.time_edit)
