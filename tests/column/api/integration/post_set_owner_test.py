...
def test_insufficient_permissions(auth_server, column_curated):
    "Check correct status code for normal users for a curated column."
    server, cookies = auth_server
    rsp = req.post_owner(
        server.url, column_curated.id_persistent, cu.test_uuid1, cookies=cookies
    )
    assert rsp.status_code == 403


def test_set_owner_non_curated(auth_server, column_user, user1):
    "Check correct status code for normal users for a curated column."
    server, cookies = auth_server
    rsp = req.post_owner(
        server.url, column_user.id_persistent, user1.id_persistent, cookies=cookies
    )
    ...
    assert ownership_request.petitioner == column_user.owner
    assert ownership_request.receiver == user1


def test_replaces_existing_request(auth_server, column_user, user1, user_commissioner):
...
def test_curated_commissioner(auth_server_commissioner, user, column_curated):
    "Test wether a commissioner can make an ownership request for a curated column."
    server, cookies = auth_server_commissioner
    rsp = req.post_owner(
        server.url, column_curated.id_persistent, user.id_persistent, cookies
    )
    assert rsp.status_code == 200
    OwnershipRequestDb.objects.filter(  # pylint: disable=no-member
...
def test_curated_commissioner_to_self(
    ...
    column = ColumnDb.most_recent_by_id(column_curated.id_persistent)
    assert column.owner == user_commissioner
