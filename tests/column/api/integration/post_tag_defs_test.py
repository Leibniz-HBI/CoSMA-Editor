...
def test_id_no_version(auth_server, root_column):
    ...
    root_column["id_persistent"] = c.id_column_parent_persistent_test
    req = r.post_column(live_server.url, root_column, cookies=cookies)
    assert req.status_code == 400
    assert (
        req.json()["msg"] == "Column with id_persistent "
        f"{c.id_column_parent_persistent_test} has no previous version."
    )


def test_no_id_version(auth_server, root_column):
    live_server, cookies = auth_server
    root_column["version"] = 5
    req = r.post_column(live_server.url, root_column, cookies=cookies)
    assert req.status_code == 400
    assert (
        req.json()["msg"]
...
def test_concurrent_modification(auth_server, root_column):
    live_server, cookies = auth_server
    req = r.post_column(live_server.url, root_column, cookies=cookies)
...
def test_no_modification_is_returned(auth_server, root_column):
    live_server, cookies = auth_server
    req = r.post_column(live_server.url, root_column, cookies=cookies)
    assert req.status_code == 200
    created = req.json()["column_list"][0]
...
def test_exists(auth_server, root_column):
    live_server, cookies = auth_server
    mock = MagicMock()
...
def test_name_exists(auth_server, root_column):
    live_server, cookies = auth_server
    req = r.post_column(live_server.url, root_column, cookies=cookies)
    assert req.status_code == 200
...
def test_self_parent(auth_server, root_column):
    live_server, cookies = auth_server
    req = r.post_column(live_server.url, root_column, cookies=cookies)
    assert req.status_code == 200
    rsp_column = req.json()["column_list"][0]
    id_persistent = rsp_column["id_persistent"]
    version = rsp_column["version"]
    new_column = root_column.copy()
    new_column["id_persistent"] = id_persistent
    new_column["id_parent_persistent"] = id_persistent
    new_column["version"] = version
    req = r.post_column(live_server.url, new_column, cookies=cookies)
    assert req.status_code == 400
    assert req.json()["msg"] == ("Can not set a column as its own parent.")


def test_no_permissions(auth_server1, root_column):
    ...
    new_column["id_persistent"] = id_persistent
    new_column["version"] = version
    new_column["name"] = "changed name"
    req = r.post_column(live_server.url, new_column, cookies=cookies_user_1)
    assert req.status_code == 403


def test_change_description(auth_server, root_column):
    ...
    new_column["id_persistent"] = id_persistent
    new_column["version"] = version
    new_column["description"] = test_description
    req = r.post_column(live_server.url, new_column, cookies=cookies)
    assert req.status_code == 200
    assert req.json()["column_list"][0]["description"] == test_description


def test_disable(auth_server, root_column):
    live_server, cookies = auth_server
    req = r.post_column(live_server.url, root_column, cookies=cookies)
    assert req.status_code == 200
    created = req.json()["column_list"][0]
...
def test_change_type(auth_server, root_column):
    live_server, cookies = auth_server
    req = r.post_column(live_server.url, root_column, cookies=cookies)
    assert req.status_code == 200
    created = req.json()["column_list"][0]
...
def test_no_parent(auth_server, child_column):
    live_server, cookies = auth_server
    child_column["id_parent_persistent"] = "unknown_id_persistent_test"
...
def test_unknown_type(auth_server, root_column):
    live_server, cookies = auth_server
    root_column["type"] = "UNKNOWN"
    req = r.post_column(live_server.url, root_column, cookies=cookies)
    assert req.status_code == 400
    assert req.json()["msg"] == "Type UNKNOWN is not known."


def test_bad_db(auth_server, root_column):
    live_server, cookies = auth_server
    mock = MagicMock()
