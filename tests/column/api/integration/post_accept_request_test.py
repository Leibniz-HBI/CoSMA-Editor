...
def test_correct_user(auth_server1, ownership_request_user):
    ...
    column = Column.most_recent_by_id(ownership_request_user.id_column_persistent)
    assert column.owner == ownership_request_user.receiver
...
def test_correct_user_curated(auth_server, ownership_request_curated):
    ...
    for a previously curated column and ownership is transferred."""
    server, cookies = auth_server
    rsp = req.post_accept(
        server.url, c.id_ownership_request_curated_test, cookies=cookies
    )
    assert rsp.status_code == 200
    column = Column.most_recent_by_id(ownership_request_curated.id_column_persistent)
    assert column.owner == ownership_request_curated.receiver
    assert not column.curated
